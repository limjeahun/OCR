...
import { DriverLicenseData } from '@/services/ocr/types';
...
export const DriverLicenseForm: React.FC<Props> = ({ data }) => {
    const [formData, setFormData] = React.useState<DriverLicenseData>(data);

    useEffect(() => {
        setFormData(data);
    }, [data]);

    const handleChange = (key: keyof DriverLicenseData, value: string) => {
